...
function arcOver({ paper, path, center, radius, startAngle, endAngle }) {
    ...
    path.arcTo(center.add(new paper.Point({
        length: radius,
        angle: startAngle + sweepAngle
    })), new paper.Size(radius, radius), 
    // @ts-ignore
    sweepAngle, isPositive, isOver180);
    // path.strokeColor = 'blue';
    path.strokeWidth = 10;
    return path;
}
export function makeConicSection({ paper, topCircumference, bottomCircumference, height, widthOffset = 0, heightOffset = 0, filletRadius = null }) {
    ...
    path.lineTo(getArcPoint({ paper, center, radius: Q - heightOffset, angle: alpha.degrees - widthOffsetAngle.degrees }));
    arcOver({
        paper,
        path,
