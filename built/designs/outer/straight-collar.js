...
export class StraightCollarOuter {
    ...
    make(paper, options) {
        var { height, neckSize, safeBorderWidth, debug = false } = options[this.constructor.name];
        ...
        // @ts-ignore
        new paper.Rectangle(0, 0, totalLength, height), height / 2);
        let curPos = 0;
        ...
        // @ts-ignore
        new paper.Rectangle(new paper.Point(curPos, height / 2 - slotHeight), new paper.Point(curPos + slotLength, height / 2 + slotHeight)), slotHeight);
        ...
        for (let h = 0; h < numHoles; h++) {
            holes.push(new paper.Path.Circle(new paper.Point(totalLength - endPadding - holeDistance * h, height / 2), BeltHoleRadius));
        }
        models.holes = holes;
        const safeAreaPadding = 0.5;
        const safeAreaLength = totalLength - beltAreaLength - holesAreaLength - safeAreaPadding * 2;
        const safeArea = new paper.Path.Rectangle(new paper.Rectangle(beltAreaLength + safeAreaPadding, safeBorderWidth, safeAreaLength, height - safeBorderWidth * 2));
        safeArea.strokeColor = new paper.Color('green');
        safeArea.strokeWidth = 0.001;
        // safeArea.remove();
        const safeCone = new paper.Path.Rectangle(new paper.Rectangle(beltAreaLength + safeAreaPadding, -height * 2, safeAreaLength, height * 4));
        safeCone.remove();
        // TODO
        // round belt end
...
//# sourceMappingURL=straight-collar.js.map