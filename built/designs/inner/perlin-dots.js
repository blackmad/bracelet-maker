...
export class InnerDesignPerlinDots extends FastAbstractInnerDesign {
    ...
    makeDesign(paper, params) {
        const { shapeSize, bufferWidth, xNoiseCoefficient, yNoiseCoefficient, shapeName, boundaryModel, chanceProbability } = params;
        let models = [];
        ...
        function makeRow(c) {
            ...
            for (var r = -1; r <= rows + 1; r++) {
                const chance = SimplexNoiseUtils.noise2DInRange(this.simplex, r * xNoiseCoefficient, c * yNoiseCoefficient, 0, 1);
                ...
                if (shapeName == 'AlternatingTriangles') {
                    ...
                    path.translate(new paper.Point(boundaryModel.bounds.x + fakeRow * (bufferWidth / 2 + shapeSize / 2), boundaryModel.bounds.y + c * (bufferWidth + shapeSize)));
                }
                else {
                    path = ShapeMaker.makeShape(paper, shapeName, shapeSize, shapeSize);
                    path.translate(new paper.Point(boundaryModel.bounds.x + r * gridCellSizeX, boundaryModel.bounds.y + c * gridCellSizeY));
                }
                rowModels.push(path);
            }
            return rowModels;
...
//# sourceMappingURL=perlin-dots.js.map