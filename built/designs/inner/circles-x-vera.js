...
export class InnerDesignCirclesXVera extends FastAbstractInnerDesign {
    ...
    makeDesign(paper, params) {
        const { boundaryModel, cols, rows, minCircleSize, maxCircleSize, borderSize, yOffset, colOffset, rowOffset, centerXNoiseDenom1, centerXNoiseDenom2, centerYNoiseDenom1, centerYNoiseDenom2, patternNoiseInfluence } = params;
        ...
        for (var r = 0; r < rows; r++) {
            for (var c = 0; c <= cols; c++) {
                const center = new paper.Point((r % 2) * rowOffset * rowCellSize + boundaryModel.bounds.x +
                    c * rowCellSize +
                    patternNoiseInfluence *
                        SimplexNoiseUtils.noise2DInRange(this.simplex, c / centerXNoiseDenom1, c / centerXNoiseDenom2, -rowCellSize * 2, rowCellSize * 2), yOffset + boundaryModel.bounds.y +
                    (c % 2) * colOffset +
                    (r * widthCellSize +
                        patternNoiseInfluence *
                            SimplexNoiseUtils.noise2DInRange(this.simplex, r / centerYNoiseDenom1, r / centerYNoiseDenom2, -widthCellSize * 2, widthCellSize * 2)));
                const circleSize = SimplexNoiseUtils.noise2DInRange(this.simplex, r / 10, c / 10, minCircleSize, maxCircleSize);
                const possibleCircle = new paper.Path.Circle(center, circleSize);
                const shouldUseCircle = possibleCircle.isInside(boundaryModel.bounds) ||
...
//# sourceMappingURL=circles-x-vera.js.map