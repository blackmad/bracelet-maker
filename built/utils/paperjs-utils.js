...
export function randomPointInPolygon(paper, polygon, rng) {
    let found = false;
    while (!found) {
        const testPoint = new paper.Point(polygon.bounds.x + rng() * polygon.bounds.width, polygon.bounds.y + rng() * polygon.bounds.height);
...
export function bufferShape(paper, buffer, points) {
    ...
    const roundedPolygon = new paper.Path(roundedShape.paths[0].map((p) => {
        return new paper.Point(p.X / 100, p.Y / 100);
    }));
    roundedPolygon.closePath();
    return roundedPolygon;
}
export function pickPointOnRectEdge(paper, box, rng) {
    ...
    else if (randomPoint > box.height + box.width &&
        randomPoint < box.height * 2 + box.width) {
        return new paper.Point(box.width + box.x, randomPoint - (box.width + box.height) + box.y);
    }
    else {
        return new paper.Point(box.width - (randomPoint - (box.height * 2 + box.width)) + box.x, box.height + box.y);
    }
}
export function randomLineEndpointsOnRectangle(paper, model, rng) {
...
export function randomLineOnRectangle(paper, model, rng) {
...
export function checkCircleCircleIntersection(c1, c2, border) {
...
//# sourceMappingURL=paperjs-utils.js.map