...
class PostureCollarImpl {
  ...
  constructor(innerDesignClass: ModelMaker, options) {
    var {
      ...
      safeBorderWidth,
      debug = false
    } = options["PostureCollar"];
    
    this.models.boltRectangle = new makerjs.models.Rectangle(neckSize + 4, 2);
...
export class PostureCollar implements ModelMaker {
  ...
  make(options): MakerJs.IModel {
    return new PostureCollarImpl(this.innerDesignClass, options);
  }
}
