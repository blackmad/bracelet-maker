...
export class InnerDesignCirclePackingImpl extends FastAbstractInnerDesign {
  makeDesign(params) {
    ...
    const lines = [
      new makerjs.paths.Line(boundaryMeasure.low, boundaryMeasure.high),
      ...
      for (let i = 0; i < triesPerRadius; i++) {
        ...
        if (
          ...
          _.every(
            circles,
            c =>
              !MakerJsUtils.checkCircleCircleIntersection(
                c,
                testCircle,
                borderSize
              )
