...
export class InnerDesignVera extends FastAbstractInnerDesign {
    constructor() {
        ...
        this.requiresSafeConeClamp = false;
        this.needSubtraction = true;
    }
    makeDesign(paper, params) {
        const { shapeSize1, shapeSize2, bufferWidth, xNoiseCoefficient, yNoiseCoefficient, xScaleNoiseCoefficient, yScaleNoiseCoefficient, minScale, maxScale, shapeName, constrainShapes, boundaryModel } = params;
        let models = [];
        ...
        function makeRow(c) {
            ...
            for (var r = 0; r < rows; r++) {
                ...
                path.rotate(SimplexNoiseUtils.noise2DInRange(this.simplex, r * xNoiseCoefficient, c * yNoiseCoefficient, 0, 180), new paper.Point(shapeSize1 / 2, shapeSize2 / 2));
                path.scale(SimplexNoiseUtils.noise2DInRange(this.simplex, r * xScaleNoiseCoefficient, c * yScaleNoiseCoefficient, minScale, maxScale));
                path.translate(new paper.Point(r * gridCellSizeX +
                    SimplexNoiseUtils.noise2DInRange(this.simplex, r * xNoiseCoefficient, c * yNoiseCoefficient, -bufferWidth / 2, bufferWidth / 2), c * gridCellSizeY +
                    SimplexNoiseUtils.noise2DInRange(this.simplex, r * xNoiseCoefficient, c * yNoiseCoefficient, -bufferWidth / 2, bufferWidth / 2)));
