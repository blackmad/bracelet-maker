...
export class InnerDesignMondrian extends FastAbstractInnerDesign {
    constructor() {
        super(...arguments);
        this.rects = [];
        this.needSubtraction = false;
        this.allowOutline = true;
    }
    splitRect(lo, hi, depth = 0) {
        const self = this;
        function makeThisRect() {
            const rect = new self.paper.Path.Rectangle(new self.paper.Rectangle(lo[0] + self.borderSize / 2, lo[1] + self.borderSize / 2, hi[0] - lo[0] - self.borderSize, hi[1] - lo[1] - self.borderSize));
    ...
    makeDesign(paper, params) {
        const { boundaryModel, borderSize, maxDepth, splitChance, xyBias, minCellSize } = params;
        this.paper = paper;
        this.rects = [];
        ...
        this.splitRect([boundaryModel.bounds.topLeft.x - borderSize / 2, boundaryModel.bounds.topLeft.y - borderSize / 2], [boundaryModel.bounds.bottomRight.x + borderSize / 2, boundaryModel.bounds.bottomRight.y + borderSize / 2], 0);
        return this.rects;
    }
