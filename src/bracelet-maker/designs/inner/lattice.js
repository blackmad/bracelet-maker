...
export class InnerDesignLattice extends FastAbstractInnerDesign {
    ...
    makeDesign(paper, params) {
        const { boundaryModel, shapeHeight, shapeWidth, borderSize, yOffset, colOffset, rowOffset } = params;
        ...
        for (var r = 0; r < rows; r++) {
            for (var c = 0; c <= cols; c++) {
                const center = new paper.Point(((r % 2) * rowOffset * shapeWidth) + boundaryModel.bounds.x +
                    (c * shapeWidth), yOffset + boundaryModel.bounds.y +
                    (c % 2) * colOffset +
                    (r * shapeHeight));
